/**
 * AI Labor Tracker - Doubao/豆包 (ByteDance) Content Script
 * Monitors Doubao chat interactions and tracks token usage
 */
...
// Doubao model name (default, may be updated from page)
...
function getConversationId(): string | undefined {
  const match = window.location.pathname.match(/\/chat\/([^/]+)/) ||
                window.location.search.match(/[?&]conversation_id=([^&]+)/);
  return match?.[1];
}
...
function detectModelName(): string {
  ...
  if (title.includes('豆包') || title.includes('Doubao')) {
    if (title.includes('Pro')) return 'Doubao-Pro';
    if (title.includes('Lite')) return 'Doubao-Lite';
    return 'Doubao';
  }
...
async function processCompletedResponse(input: string, output: string): Promise<void> {
  ...
  try {
    ...
    console.log('[AI Labor Tracker] Doubao interaction captured:', {
      inputTokens,
      outputTokens,
      model,
    });
...
function setupMutationObserver(): void {
  ...
  console.log('[AI Labor Tracker] Doubao mutation observer initialized');
}
  ...
  try {
    ...
    if (response?.settings) {
      isCapturing = response.settings.captureEnabled && 
                    response.settings.enabledPlatforms?.doubao !== false;
    }
  } catch (error) {
...
async function init(): Promise<void> {
  console.log('[AI Labor Tracker] Doubao content script loaded');
  
  // Check capture status
  await checkCaptureStatus();
  
  if (!isCapturing) {
    console.log('[AI Labor Tracker] Doubao capture disabled');
    return;
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.type === 'SETTINGS_UPDATED') {
      isCapturing = message.payload?.captureEnabled !== false && 
                    message.payload?.enabledPlatforms?.doubao !== false;

/**
 * AI Labor Tracker - DeepSeek Content Script
 * Monitors DeepSeek chat interactions and tracks token usage
 */
...
// DeepSeek model name (default, may be updated from page)
...
function detectModelName(): string {
  ...
  if (title.includes('DeepSeek')) {
    if (title.includes('V3')) return 'DeepSeek-V3';
    if (title.includes('Coder')) return 'DeepSeek-Coder';
    if (title.includes('Chat')) return 'DeepSeek-Chat';
  }
...
function findUserMessages(): Element[] {
  const selectors = [
    ...
    '.user-bubble',
    '[class*="human"]',
    'div[class*="fbb737a4"]', // DeepSeek specific user class pattern
  ];
...
function findAIResponses(): Element[] {
  const selectors = [
    ...
    '.assistant-bubble',
    '[class*="bot"]',
    '[class*="markdown"]', // DeepSeek often wraps responses in markdown
    'div[class*="ds-markdown"]', // DeepSeek specific markdown class
    'div[class*="a6d716e5"]', // DeepSeek specific assistant class pattern
  ];
...
async function processCompletedResponse(input: string, output: string): Promise<void> {
  ...
  try {
    ...
    console.log('[AI Labor Tracker] DeepSeek interaction captured:', {
      inputTokens,
      outputTokens,
      model,
    });
...
function setupMutationObserver(): void {
  ...
  console.log('[AI Labor Tracker] DeepSeek mutation observer initialized');
}
  ...
  try {
    ...
    if (response?.settings) {
      isCapturing = response.settings.captureEnabled && 
                    response.settings.enabledPlatforms?.deepseek !== false;
    }
  } catch (error) {
...
async function init(): Promise<void> {
  console.log('[AI Labor Tracker] DeepSeek content script loaded');
  
  // Check capture status
  await checkCaptureStatus();
  
  if (!isCapturing) {
    console.log('[AI Labor Tracker] DeepSeek capture disabled');
    return;
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.type === 'SETTINGS_UPDATED') {
      isCapturing = message.payload?.captureEnabled !== false && 
                    message.payload?.enabledPlatforms?.deepseek !== false;

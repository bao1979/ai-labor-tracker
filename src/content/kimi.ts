/**
 * AI Labor Tracker - Kimi (Moonshot) Content Script
 * Monitors Kimi chat interactions and tracks token usage
 */
...
// Kimi model name (default, may be updated from page)
...
async function processCompletedResponse(input: string, output: string): Promise<void> {
  ...
  try {
    ...
    console.log('[AI Labor Tracker] Kimi interaction captured:', {
      inputTokens,
      outputTokens,
      model,
    });
...
function setupMutationObserver(): void {
  ...
  console.log('[AI Labor Tracker] Kimi mutation observer initialized');
}
  ...
  try {
    ...
    if (response?.settings) {
      isCapturing = response.settings.captureEnabled && 
                    response.settings.enabledPlatforms?.kimi !== false;
    }
  } catch (error) {
...
async function init(): Promise<void> {
  console.log('[AI Labor Tracker] Kimi content script loaded');
  
  // Check capture status
  await checkCaptureStatus();
  
  if (!isCapturing) {
    console.log('[AI Labor Tracker] Kimi capture disabled');
    return;
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.type === 'SETTINGS_UPDATED') {
      isCapturing = message.payload?.captureEnabled !== false && 
                    message.payload?.enabledPlatforms?.kimi !== false;

/**
 * AI Labor Tracker - Qianwen/通义千问 (Tongyi) Content Script
 * Monitors Qianwen chat interactions and tracks token usage
 */
...
// Qianwen model name (default, may be updated from page)
let currentModel = 'Qianwen';
...
function detectModelName(): string {
  ...
  if (title.includes('通义千问') || title.includes('Tongyi') || title.includes('Qwen')) {
    if (title.includes('Max')) return 'Qwen-Max';
    if (title.includes('Plus')) return 'Qwen-Plus';
    if (title.includes('Turbo')) return 'Qwen-Turbo';
    return 'Qianwen';
  }
...
async function processCompletedResponse(input: string, output: string): Promise<void> {
  ...
  try {
    ...
    console.log('[AI Labor Tracker] Qianwen interaction captured:', {
      inputTokens,
      outputTokens,
      model,
    });
...
function setupMutationObserver(): void {
  ...
  console.log('[AI Labor Tracker] Qianwen mutation observer initialized');
}
  ...
  try {
    ...
    if (response?.settings) {
      isCapturing = response.settings.captureEnabled && 
                    response.settings.enabledPlatforms?.qianwen !== false;
    }
  } catch (error) {
...
async function init(): Promise<void> {
  console.log('[AI Labor Tracker] Qianwen content script loaded');
  
  // Check capture status
  await checkCaptureStatus();
  
  if (!isCapturing) {
    console.log('[AI Labor Tracker] Qianwen capture disabled');
    return;
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.type === 'SETTINGS_UPDATED') {
      isCapturing = message.payload?.captureEnabled !== false && 
                    message.payload?.enabledPlatforms?.qianwen !== false;
